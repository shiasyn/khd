...
internal struct mode *active_mode;
...
internal EVENT_TAP_CALLBACK(key_handler)
{
    switch(type)
    {
        ...
        case kCGEventKeyDown:
        {
            ...
            bool result = find_and_exec_hotkey(&eventkey, &mode_map);
            if(result) {
                return NULL;
            }
        } break;
...
set_config_path()
{
    if(!config_file) {
        ...
        if(home) {
            ...
            strcat(config_file, "/.khdrc");
        } else {
            config_file = strdup(".khdrc");
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    if(getuid() == 0 || geteuid() == 0) {
        error("khd: running as root is not allowed! abort..\n");
    }

    if(secure_keyboard_entry_enabled()) {
        error("khd: secure keyboard entry is enabled! abort..\n");
    }

    if(!check_privileges()) {
        error("khd: must be run with accessibility access.\n");
    }
    ...
    active_mode = table_find(&mode_map, "default");
